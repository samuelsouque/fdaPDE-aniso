...
template <class Derived, typename InputHandler, typename Integrator, UInt ORDER>
std::vector<Real> AnisotropicSmoothingBase<Derived, InputHandler, Integrator, ORDER>::seq(const Real & start, const Real & end, const Real & by) const {
    ...
    std::transform(result.begin(), result.end(), result.begin(), [] (const Real & el) -> Real { return el/(1-el)*2; });
    return result;
}
